...
<section class="section">
    ...
    <x-card title="Daftar Kategori" :button="['type' => 'button', 'class' => 'btn btn-sm btn-primary rounded-pill', 'dataBsToggle' => 'modal', 'dataBsTarget' => '#buatKategori', 'label' => 'Tambah Kategori']">
        <x-modal id="buatKategori" title="Tambah Kategori">
            <x-form class="" action="{{ route('admin.blog-category.store') }}" method="POST">
                @csrf
                <div class="col-md-12">
                    <x-input type="text" label="Nama Kategori" name="name" required=true  placeholder="Peternakan" />
                </div>
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <!-- Hapus Kategori Modal -->
                             ...
                             <div class="modal-footer">
                                ...
                                <form action="{{ route('admin.blog-category.delete', $category->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
